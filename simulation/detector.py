...
def create_detector_response(filename,image):
    """
    ...
    @param filename: path to csv file with comma separated x and y value (x,y) on each line
    ...
    r = xp.sqrt(R ** 2 + Y ** 2)
    response = sinc_square(r,params[0],params[1],params[2],params[3])
...
name = f'{detector}_{function}_{int(voltage*E-3)}kV'

filename = f'{folder}/{name}.csv'

dqe = create_detector_response(filename,xp.zeros((256,256)))

filename = '/data2/mchaillet/simulation/detectors/FALCONII_MTF_300kV.csv'

mtf = create_detector_response(filename,xp.zeros((256,256)))
...
# X-axis is in units of angular frequency, and a fraction of Nyquist frequency
# xdata, ydata = read_csv(filename)
#
# params = fit_sinc_square(xdata,ydata)
#
# xaxis = xp.linspace(0,1,100) # we can plot with xdata, but fit will not look good
# curve_y = sinc_square(xaxis,params[0],params[1],params[2],params[3])
