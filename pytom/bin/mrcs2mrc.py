...
if __name__ == '__main__':
    ...
    options = [ScriptOption(['-f', '--fileName'], 'Filename of mrc stack.', True, False),
               ...
               ScriptOption(['-o', '--origDir'], 'Directory from which images in the stack originate. '
                            ...
                            True, True),
               ScriptOption(['-i', '--angularIncrement'],
                            'Angular Increment between tilts (typically 2 or 3 degrees). Default 2 degrees.', True,
                            True),
               ...
               ScriptOption(['--metaFile'], 'Pytom-specific meta file containing the tiltangles of the images.', True,
                            True),
    ...
    try:
        filename, outdir, prefix, origdir, increment, startAngle, endAngle, metafile, mdoc, help = parse_script_options(
            sys.argv[1:], helper)
    ...
    if mdoc:
        mdocname = os.path.join(outdir,
                                os.path.basename(filename).replace('.mrc', '.mdoc').replace('.ali', '.mdoc').replace(
                                    '.st', '.mdoc'))
        print(mdocname)
...
#
# '''
#     if os.path.exists(mdocfilename):
#         mdoc = True
#         mdocfile = open(mdocfilename, 'w')
#     else:
#         mdoc = False
#
#     if origdir:
