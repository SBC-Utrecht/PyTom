...
/* 
   ...
   FUNCTIONS WILL WRITE OVER DATA
   ...
   % test_FST_hybrid_memoX bw loops [error_file]
...
void main(int argc, char **argv)
{
  ...
  if (argc < 3)
    {
      fprintf(stdout,"Usage: test_FST_hybrid_memoX bw loops [error_file]\n");
      exit(0);
    }
  ...
  ***/
  ...
  fprintf(stderr,"Program: test_FST_hybrid_memoX\n");
  ...
  free(cos_size); free(curmax); free(relerror); free(workspace);
  free(poly); free(trans_seminaive_table); free(seminaive_table);
