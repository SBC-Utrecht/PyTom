...
/******************************************************************
  ...
  CONV_SEMI_FLY sphere_bw64.dat filter_bw64.dat 64
...
void main(int argc, char **argv)
{
  ...
  if (argc != 4)
    {
      fprintf(stdout,"Usage: CONV_SEMI_FLY signal_file filter_file bw\n");
      exit(0);
    }
  ...
  Conv2Sphere_semi_fly(rsignal, isignal, 
		       rfilter, ifilter, 
		       rresult, iresult, 
		       size,
		       workspace);
#else
  Conv2Sphere_semi_fly(rsignal,
		       rfilter,
		       rresult,
		       size,
		       workspace,
