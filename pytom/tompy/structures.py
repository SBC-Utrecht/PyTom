...
class Preprocessing(PyTomClass):
    ...
    def apply(self, volume, bypassFlag=False, downscale=1, particle=None):
        ...
        if self._weightingOn and (not bypassFlag):
            ...
            from pytom.tompy.structures import Weight as weight
...
class Mask(PyTomClass):
    ...
    def getVolume(self, rotation=None, bufferedRead=False):
        ...
        if rotation and not (self._isSphere):
            from pytom.voltools import transform
            from pytom.tompy.tools import determineRotationCenter

            if rotation.__class__ == list:
                from pytom.tompy.structures import Rotation
...
class Reference(PyTomClass):
    ...
    def subtractParticle(self, particle, binning=1, verbose=False):
        ...
        from pytom.tompy.filter import rotateWeighting
        ...
        # bin to size
        if binning > 1:
            from pytom.tompy.transform import resize  # , scale as scaleVolume
            from pytom.tompy.io import readSubvolumeFromFourierspaceFile
...
class Wedge(PyTomClass):
    ...
    def __init__(self, wedgeAngles=[0.0, 0.0], cutoffRadius=0.0, tiltAxis='Y', smooth=0.0):
        ...
        from pytom.tompy.structures import DoubleTiltWedge, SingleTiltWedge
...
be generated. If omitted / 0, filter is fixed to size/2.
        @param smooth: Smoothing size of wedge at the edges in degrees. Default is 0.
        """
        from pytom.tompy.structures import SingleTiltWedge
        tilt1 = wedgeAngles[0]
        tilt2 = wedgeAngles[1]
...
class Particle(PyTomClass):
    ...
    def fromXML(self, xmlObj):
        ...
        from lxml.etree import _Element
        from pytom.tompy.structures import Rotation, Shift, PickPosition, Wedge
    ...
    def getTransformedVolume(self, binning=1):
        ...
        @rtype: L{pytom_volume.vol}
        """
        from pytom.tompy.structures import Shift, Rotation
        from pytom.voltools import transform
...
class ParticleList(PyTomClass):
    ...
    def loadDirectory(self, av3Indexing=False, av3Prefix=''):
        ...
        """
        import dircache
        from pytom.tompy.structures import Particle
        from pytom.tools.files import checkDirExists
    ...
    def fromMOTL(self, motlFile, sourceDirectory, particlePrefix, originVolume='', particleStartOffset=0, score=None):
        ...
        from pytom.tompy.structures import Rotation, Shift, PickPosition, Particle
        # if not score:
        if score == None:
    ...
    def particlesFromClass(self, className):
        ...
        @return: ParticleList
        """
        from pytom.tompy.structures import ParticleList as ParticleListConstructor
...
class Shift(PyTomClass):
    ...
    def __len__(self):
        from pytom.tompy.tools import euclidian_distance
...
class PointSymmetry(Symmetry):
    ...
    def apply(self, particleList):
        ...
        from pytom.tompy.structures import ParticleList, Rotation
    ...
    def calculateCorrelationProfile(self, volume, mask=None, axis='Z', angularIncrement=1, plot=False, verbose=False,
        ...
        from pytom.voltools import StaticVolume, transform
