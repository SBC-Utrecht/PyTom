...
# TODO: we move .git only to make sure we can do "git submodule update", there must be a better way
ADD .git /app/.git
ADD pytom /app/pytom
ADD tests /app/tests
...
# to run tests
# option 1, Gijs' test runner
#RUN ["pytom", "tests/runPyTomUnitTests.py"]
# option 2, auto discover tests in the current directory
#WORKDIR /app/tests
#RUN ["pytom", "-m", "unittest", "discover"]