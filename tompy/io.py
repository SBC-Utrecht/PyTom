...
def write(filename, data, tilt_angle=0):
...
def read_em_header(filename): # TODO
    f = open(filename, 'rb')
    try:
...
def read_em_tilt_angle(filename):
    ...
    try:
        header_data = np.fromfile(f, np.dtype('int32'), 128)
        tilt_angle = header_data[24+18]/1000.
    finally:
        f.close()
...
def write_em_tilt_angle(filename, tilt_angle):
    header = read_em_header(filename)
    header[24+18] = int(tilt_angle*1000)
    data = read(filename)
    f = open(filename, 'wb')
    try:
        f.write(header.tostring())
