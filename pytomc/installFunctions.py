...
def parseArguments(args):
    ...
    if minconIndex != None:
        ...
        for f in ('c++', 'cc' ):
            fname = os.path.join(args[minconIndex+1],'x86_64-conda_cos6-linux-gnu-'+f)
            fnm = os.path.join(args[minconIndex+1],f)
            if os.path.exists(fname): os.system(f'ln -s {fnm} {fname}')
