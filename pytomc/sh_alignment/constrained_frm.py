    ...
    def fromXML(self, xml_obj):
        try:
            ...
            if type == str(AngularConstraint.FIXED_ANGLE):
                ...
                c = FixedAxisConstraint(0,0,1,0)
            elif type == str(AngularConstraint.ADP_ANGLE):
                c = AdaptiveAngleConstraint(0)
            else:
...
class FixedAxisConstraint(AngularConstraint):
    ...
    def __init__(self, x, y, z, nearby=0, range=(0,360,2)):
        ...
        self.nearby = nearby # this parameter allows the axis to be flexible
        self.range = range
    ...
    def toXML(self):
        ...
        xml_obj = etree.Element('AngularConstraint', Type=str(self.type), X=str(self.x), Y=str(self.y), Z=str(self.z), Nearby=str(self.nearby), Range=str(self.range))
...
class AdaptiveAngleConstraint(AngularConstraint):
    """docstring for AdaptiveAngleConstraint"""
    def __init__(self, nearby):
        """
        """
        super(AdaptiveAngleConstraint, self).__init__(AngularConstraint.ADP_ANGLE)
    ...
    def setAngle(self, phi, psi, the):
        """Consistent with PyTom notation: Z1 Z2 X
        """
        self.phi = phi
        self.psi = psi
        self.the = the
        self._cv = None # set it explicit to None
    ...
    def toXML(self):
        ...
        xml_obj = etree.Element('AngularConstraint', Type=str(self.type), Nearby=str(self.nearby))
